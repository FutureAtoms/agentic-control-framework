...
// Modified Gemini Task Manager MCP server implementation
// This version reads workspace root from config.json
...
// Get workspace root from configuration file
let currentWorkspaceRoot = null;
try {
  const configFilePath = path.join(process.cwd(), '.task_manager', 'config.json');
  if (fs.existsSync(configFilePath)) {
    const config = JSON.parse(fs.readFileSync(configFilePath, 'utf8'));
    if (config.workspaceRoot) {
      currentWorkspaceRoot = config.workspaceRoot;
      console.error(`Using workspace root from config file: ${currentWorkspaceRoot}`);
    }
  }
  
  // If not found in config, try command line args
  ...
  // If still not found, use process.cwd()
  if (!currentWorkspaceRoot) {
    currentWorkspaceRoot = process.cwd();
    console.error(`Falling back to current directory as workspace root: ${currentWorkspaceRoot}`);
  }
} catch (error) {
  console.error(`[ERROR] Failed to load workspace configuration: ${error.message}`);
  currentWorkspaceRoot = process.cwd();
  console.error(`Falling back to current directory as workspace root: ${currentWorkspaceRoot}`);
}
      ...
      case 'tools/call':
        ...
        if (!currentWorkspaceRoot) {
             ...
             sendError(id, -32001, "Server Configuration Error", { 
               message: "Workspace root could not be determined. Cannot execute tool." 
             });
            return;
        }
